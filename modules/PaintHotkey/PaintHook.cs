...
namespace TinyTools
{
    ...
    {
        private static readonly Lazy<PaintHook> instance = new(() => new PaintHook());
        public static PaintHook Instance => instance.Value;
        ...
        public void StartHook()
        {
            if (hookId == IntPtr.Zero)
            {
                ...
                {
                    Console.WriteLine("✓ Paint hotkey monitoring started (Ctrl+W)");
                }
            }
        }
        ...
        public void StopHook()
        {
            if (hookId != IntPtr.Zero)
            {
                ...
                hookId = IntPtr.Zero;
                Console.WriteLine("✗ Paint hotkey monitoring stopped");
            }
        }
        ...
        private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                {
                    if (ctrlPressed && IsPaintActive())
                    {
                        var activeWindow = GetForegroundWindow();
                        if (activeWindow != IntPtr.Zero)
                        {
                            PostMessage(activeWindow, 0x0010, IntPtr.Zero, IntPtr.Zero); // WM_CLOSE
                            Console.WriteLine("Paint window closed via Ctrl+W");
                        }
        ...
        private bool IsPaintActive()
        {
            ...
            try
            {
                ...
                {
                    return process.ProcessName.ToLower() == "mspaint";
                }
            }
            catch
            {
